...
def course_page(request, code):
    try:
        course = Course.objects.get(code=code)
        student_id = request.session.get('student_id')  # Getting student ID from session
        if not student_id:
            return redirect('std_login')  # Redirect to login if no session
...
def view_course_students(request, course_id):
    ...
    for student in students:
        # Sum marks from assignment submissions
        total_assignment_points_awarded = Submission.objects.filter(
            student=student, 
            assignment__course_code=course
        ).aggregate(
            total_awarded=Sum('marks')
        )['total_awarded'] or 0

        # Sum marks from correct quiz answers
        total_quiz_points_awarded = StudentAnswer.objects.filter(
            student=student,
            quiz__course=course,
            answer=F('question__answer')
        ).aggregate(
            total_awarded=Sum('question__marks')
        )['total_awarded'] or 0

        # Combine assignment and quiz points
        total_points_awarded = total_assignment_points_awarded + total_quiz_points_awarded

        # Calculate total possible points from assignments
        total_possible_assignment_points = Assignment.objects.filter(
            course_code=course
        ).aggregate(
            total_possible=Sum('marks')
        )['total_possible'] or 0

        # Calculate total possible points from quizzes
        total_possible_quiz_points = Question.objects.filter(
            quiz__course=course
        ).aggregate(
            total_possible=Sum('marks')
        )['total_possible'] or 0

        # Total possible points from both assignments and quizzes
        total_possible_points = total_possible_assignment_points + total_possible_quiz_points

        # Calculate the grade as a percentage
        grade_percentage = (total_points_awarded / total_possible_points * 100) if total_possible_points > 0 else 0
    ...
    return render(request, 'main/course_students.html', {'course': course, 'student_grades': student_grades})
